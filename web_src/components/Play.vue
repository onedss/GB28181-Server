  ...
  <div class="content-wrapper">
    <section :class="[{'content': !fullscreen}, {'no-padding': fullscreen}]">
      <div class="player-wrapper" :style="{margin:'0 auto', width: fullscreen ? '100%' : isMobile() ? '95%' : '85%' }">
        <div class="play-area">
          <LivePlayer ref="player" :hideBigPlayButton="!!serverInfo.HideBigPlayButton" :muted="muted" :videoUrl="videoUrl" :aspect="aspect" live :poster="poster"
            v-loading="bLoading" :alt="alt" :debug="debug" element-loading-text="加载中..." element-loading-background="#000" :loading.sync="bLoading" @message="$message"
...
export default {
  ...
  async mounted() {
    ...
    this.otherParams = this.getOtherParams(["aspect", "autoplay", "controls", "ptz", "share", "fluent", "stretch", "alt", "type", "starttime", "endtime", "serial", "code", "channel", "protocol", "muted", "talk", "debug"])
